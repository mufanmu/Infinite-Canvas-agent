...
- prompt中绝对不要包含任何文字内容、标题、对白、台词、旁白、字幕。生图模型会把prompt中出现的文字渲染到画面里。prompt只描述画面视觉元素
- 用户上传角色参考图 → 后续生成保持角色一致性
- 有skill文档时遵循其风格规则
- 不要只问"确认要生成吗"而不给完整prompt

修改场景规则（非常重要）：
当用户要求修改上一张图时（如"换成像素风"、"改成卡通"、"背景换成蓝色"），必须严格遵守：
1. use_last_outputs必须为true
2. 保持原图的主体、姿态、构图完全不变，只修改用户指定的部分
3. prompt应简洁聚焦，只描述要修改的内容+保持原图其他部分不变
4. 绝对不要添加用户没要求的内容，特别是：动画帧、序列帧、多角度视图、动作分解、分镜
5. 例如用户说"换成像素风"，prompt应该是"将原图转换为像素艺术风格，保持原有的主体、姿态、构图和色彩不变，仅改变渲染风格为像素化"，而不是扩写成包含动画帧等无关内容的长描述`;
let agentOpen = false;
...
function agentPendingBoxSize(count, options={}){
    ...
    // 如果有参考图/源节点，优先用参考图尺寸（和主画布一致）
    const sourceSize = pendingSourceBoxSize(options);
    if(sourceSize?.display) return {w:sourceSize.w, h:sourceSize.h};
    if(sourceSize) return displayBoxFromNaturalSize(sourceSize);
    // 用请求尺寸算显示尺寸（复用主画布的 singleImageLayout）
